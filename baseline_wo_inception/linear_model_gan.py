...
class LinearModel(object):
  ...
  def generator(self):
    ...
    with vs.variable_scope( "generator" ):
  ...
  def discriminator(self, 3d_input):

    # === Create the linear + relu combos ===
    with vs.variable_scope( "discriminator" ):

      # === First layer, brings dimensionality up to linear_size ===
      w1 = tf.get_variable( name="w1", initializer=kaiming, shape=[self.HUMAN_3D_SIZE, linear_size], dtype=dtype )
      b1 = tf.get_variable( name="b1", initializer=kaiming, shape=[linear_size], dtype=dtype )
      w1 = tf.clip_by_norm(w1,1) if max_norm else w1
      y3 = tf.matmul( 3d_input, w1 ) + b1
