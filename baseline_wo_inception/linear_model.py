...
class LinearModel(object):
  ...
  def __init__(self,
    ...
    with vs.variable_scope( "linear_model" ):
      ...
      # w = tf.get_variable( name="w", initializer=kaiming, shape=[linear_size, self.HUMAN_3D_SIZE], dtype=dtype )
      # b = tf.get_variable( name="b", initializer=kaiming, shape=[self.HUMAN_3D_SIZE], dtype=dtype )
      # w = tf.clip_by_norm(w,1) if max_norm else w
      # y4 = tf.matmul( y3, w ) + b


      # for idx1 in range(num_layers, num_layers*2 ):
      #   y3 = self.two_linear( y3, linear_size, residual, self.dropout_keep_prob, max_norm, batch_norm, dtype, idx1 )
    ...
    # self.loss_mid = tf.reduce_mean(tf.square(y4 - dec_out))
    ...
    #   gradients_mid = opt.compute_gradients(self.loss_mid, [])

    #   self.gradients = [[] if i==None else i for i in gradients]
    #   self.gradients_mid = [[] if i==None else i for i in gradients_mid]
    #   self.updates = opt.apply_gradients(gradients, global_step=self.global_step)
    #   self.updates_mid = opt.apply_gradients(gradients_mid, global_step=self.global_step)
    ...
    with tf.control_dependencies(update_ops):
      ...
      self.gradients = self.opt.compute_gradients(loss=self.loss)

      l2_norm = lambda t: tf.sqrt(tf.reduce_sum(tf.pow(t, 2)))
      self.gradients_summary = []
      self.variables_summary = []
      for gradient, variable in self.gradients:
        self.gradients_summary.append(tf.summary.histogram("gradients/" + variable.name, l2_norm(gradient)))
        self.variables_summary.append(tf.summary.histogram("variables/" + variable.name, l2_norm(variable)))

      self.train_op = self.opt.apply_gradients(self.gradients, global_step=self.global_step)

      # self.opt = tf.train.AdamOptimizer( self.learning_rate ).minimize(self.loss + self.loss_mid, global_step=self.global_step)
