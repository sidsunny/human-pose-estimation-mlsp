  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    for _ in xrange( FLAGS.epochs ):
      ...
      for i in range( nbatches ):
        ...
        step_loss, loss_summary, lr_summary, gradients_summary, variables_summary, _ =  model.step( sess, enc_in, dec_out, FLAGS.dropout, isTraining=True )

        if (i+1) % log_every_n_batches == 0:
          # Log and print progress every log_every_n_batches batches
          model.train_writer.add_summary( loss_summary, current_step )
          model.train_writer.add_summary( lr_summary, current_step )
          for g in gradients_summary:
            model.train_writer.add_summary( g, current_step )
          for v in variables_summary:
            model.train_writer.add_summary( v, current_step )
