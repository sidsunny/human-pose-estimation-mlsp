...
class LinearModel(object):
  ...
  def __init__(self,
    ...
    projs_3d = self.generator(enc_in, max_norm, batch_norm, num_layers_gen, residual, dtype)

    logits_real, out_real = self.discriminator(dec_out, max_norm, batch_norm, num_layers_dis, residual, dtype, reuse=not self.isTraining)
    logits_gen, out_gen = self.discriminator(projs_3d, max_norm, batch_norm, num_layers_dis, residual, dtype, reuse=True)


    smooth = 0.1
    # Store the outputs here
    self.outputs = projs_3d
    self.loss_gen = tf.reduce_mean(tf.square(projs_3d - dec_out))
    self.loss_gen_summary = tf.summary.scalar('loss_gen/loss_gen', self.loss_gen)
    self.loss_dis_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = logits_real, labels = tf.ones_like(logits_real)*(1-smooth)))
    self.loss_dis_gen = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = logits_gen, labels = tf.zeros_like(logits_gen)))
    self.loss_dis = self.loss_dis_real + self.loss_dis_gen
    self.loss_dis_summary = tf.summary.scalar('loss_dis/loss_dis', self.loss_dis)

    # To keep track of the loss in mm
    self.err_mm = tf.placeholder( tf.float32, name="error_mm" )
    self.err_mm_summary = tf.summary.scalar( "loss_gen/error_mm", self.err_mm )
    ...
    with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
        self.d_train_opt = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss_dis, var_list=d_vars)
        self.g_train_opt = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss_gen, var_list=g_vars, global_step=self.global_step)

        # self.opt = tf.train.AdamOptimizer( self.learning_rate ).minimize(self.loss_mid)
  ...
  def generator(self, enc_in, max_norm, batch_norm, num_layers, residual, dtype):

    # === Create the linear + relu combos ===
    with vs.variable_scope( "generator" , reuse=not self.isTraining):

      # === First layer, brings dimensionality up to linear_size ===
      w1 = tf.get_variable( name="w1", initializer=kaiming, shape=[self.HUMAN_2D_SIZE, self.linear_size], dtype=dtype )
      b1 = tf.get_variable( name="b1", initializer=kaiming, shape=[self.linear_size], dtype=dtype )
      w1 = tf.clip_by_norm(w1,1) if max_norm else w1
      y3 = tf.matmul( enc_in, w1 ) + b1

      if batch_norm:
        y3 = tf.layers.batch_normalization(y3, name="batch_normalization")
      y3 = tf.nn.relu( y3 )
      y3 = tf.nn.dropout( y3, self.dropout_keep_prob )

      # === Create multiple bi-linear layers ===
      for idx in range( num_layers ):
        y3 = self.two_linear( y3, self.linear_size, residual, self.dropout_keep_prob, max_norm, batch_norm, dtype, idx )

      # === Last linear layer has HUMAN_3D_SIZE in output ===
      w4 = tf.get_variable( name="w4", initializer=kaiming, shape=[self.linear_size, self.HUMAN_3D_SIZE], dtype=dtype )
  ...
  def discriminator(self, input_3d, max_norm, batch_norm, num_layers, residual, dtype, reuse=False):

    # === Create the linear + relu combos ===
    with vs.variable_scope( "discriminator" , reuse=reuse):

      # === First layer, brings dimensionality up to linear_size ===
      w1 = tf.get_variable( name="w1", initializer=kaiming, shape=[self.HUMAN_3D_SIZE, self.linear_size], dtype=dtype )
      b1 = tf.get_variable( name="b1", initializer=kaiming, shape=[self.linear_size], dtype=dtype )
      w1 = tf.clip_by_norm(w1,1) if max_norm else w1
      y3 = tf.matmul( input_3d, w1 ) + b1
      ...
      # === Last linear layer has HUMAN_3D_SIZE in output ===
      w4 = tf.get_variable( name="w4", initializer=kaiming, shape=[self.linear_size, 1], dtype=dtype )
      b4 = tf.get_variable( name="b4", initializer=kaiming, shape=[1], dtype=dtype )
      w4 = tf.clip_by_norm(w4,1) if max_norm else w4
      y = tf.matmul(y3, w4) + b4
      out = tf.sigmoid(y)
      # === End linear model ===
  ...
  def two_linear( self, xin, linear_size, residual, dropout_keep_prob, max_norm, batch_norm, dtype, idx ):
    ...
    with vs.variable_scope( "two_linear_"+str(idx) ) as scope:
      ...
      if  batch_norm:
        y = tf.layers.batch_normalization(y,name="batch_normalization1"+str(idx))

      # original y = tf.nn.relu( y )
      ...
      if  batch_norm:
        y = tf.layers.batch_normalization(y,name="batch_normalization2"+str(idx))
  ...
  def step(self, session, encoder_inputs, decoder_outputs, dropout_keep_prob, isTraining=True):
    ...
    if isTraining:
      output_feed = [self.g_train_opt,
                     self.d_train_opt,
                     self.loss_gen,
                     self.loss_dis,
                     self.loss_gen_summary,
                     self.loss_dis_summary,
                     self.learning_rate_summary,
                     self.outputs]

      outputs = session.run( output_feed, input_feed )
      return outputs[2], outputs[3], outputs[4], outputs[5], outputs[6], outputs[7]

    else:
      output_feed = [self.loss_gen,
                     self.loss_dis, # Loss for this batch.
                     self.loss_gen_summary,
                     self.loss_dis_summary,
                     self.outputs]

      outputs = session.run(output_feed, input_feed)
      return outputs[0], outputs[1], outputs[2], outputs[3], outputs[4]  # No gradient norm
