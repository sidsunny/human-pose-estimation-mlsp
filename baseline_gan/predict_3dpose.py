...
tf.app.flags.DEFINE_integer("num_layers_gen", 4, "Number of layers in the generator model.")
tf.app.flags.DEFINE_integer("num_layers_dis", 4, "Number of layers in the discriminator model.")
...
tf.app.flags.DEFINE_integer("load", 386846, "Try to load a previous checkpoint.")
...
train_dir = os.path.join( FLAGS.train_dir,
  ...
  'gen_depth_{0}'.format(FLAGS.num_layers_gen),
  'dis_depth_{0}'.format(FLAGS.num_layers_dis),
...
def create_model( session, actions, batch_size ):
  ...
  model = linear_model.LinearModel(
      FLAGS.linear_size,
      FLAGS.num_layers_gen,
      FLAGS.num_layers_dis,
      FLAGS.residual,
      FLAGS.batch_norm,
      FLAGS.max_norm,
      batch_size,
...
def train():
  ...
  with tf.Session(config=tf.ConfigProto(
    ...
    # === Create the model ===
    print("Creating %d and %d bi-layers of %d units in generator and discriminator." % (FLAGS.num_layers_gen, FLAGS.num_layers_dis, FLAGS.linear_size))
    model = create_model( sess, actions, FLAGS.batch_size )
    model.train_writer.add_graph( sess.graph )
    print("Model created")

    #=== This is the training loop ===
    step_time, loss_gen_total, loss_dis_total, val_loss = 0.0, 0.0, 0.0, 0.0
    current_step = 0 if FLAGS.load <= 0 else FLAGS.load + 1
    previous_losses = []

    step_time, loss_gen_total, loss_dis_total = 0, 0, 0
    current_epoch = 0
    ...
    for _ in xrange( FLAGS.epochs ):
      ...
      start_time, loss_gen_total, loss_dis_total = time.time(), 0., 0.
      ...
      for i in range( nbatches ):
        ...
        loss_gen, loss_dis, loss_gen_summary,loss_dis_summary, lr_summary, _ =  model.step( sess, enc_in, dec_out, FLAGS.dropout, isTraining=True )

        if (i+1) % log_every_n_batches == 0:
          # Log and print progress every log_every_n_batches batches
          model.train_writer.add_summary( loss_gen_summary, current_step )
          model.train_writer.add_summary( loss_dis_summary, current_step )
      ...
      loss_gen_total = loss_gen_total / nbatches
      loss_dis_total = loss_dis_total / nbatches

      f.write("=============================\n"
            "Global step:         %d\n"
            "Learning rate:       %.2e\n"
            "Train loss_gen_total avg:      %.4f\n"
            "Train loss_dis_total avg:      %.4f\n"
            "=============================\n" % (model.global_step.eval(),
            model.learning_rate.eval(), loss_gen_total, loss_dis_total))
      ...
      if FLAGS.evaluateActionWise:
        ...
        for action in actions:
          ...
          act_err, _, step_time, loss_gen_total, loss_dis_total = evaluate_batches( sess, model,
        ...
        f.write("{0:<12} {1:>6.2f}\n".format("Average", cum_err/float(len(actions) )))
        ...
        total_err, joint_err, step_time, loss_gen_total, loss_dis_total = evaluate_batches( sess, model,
          data_mean_3d, data_std_3d, dim_to_use_3d, dim_to_ignore_3d,
          data_mean_2d, data_std_2d, dim_to_use_2d, dim_to_ignore_2d,
        ...
        print("=============================\n"
              "Step-time (ms):      %.4f\n"
              "Val loss_gen_total avg:        %.4f\n"
              "Val loss_dis_total avg:        %.4f\n"
              "Val error avg (mm):  %.2f\n"
              "=============================" % ( 1000*step_time, loss_gen_total, loss_dis_total, total_err ))
...
def evaluate_batches( sess, model,
  ...
  all_dists, start_time, loss_gen_total, loss_dis_total = [], time.time(), 0., 0.
  log_every_n_batches = 100
  for i in range(nbatches):
    ...
    loss_gen, loss_dis, loss_gen_summary, loss_dis_summary, poses3d = model.step( sess, enc_in, dec_out, dp, isTraining=False )
    loss_gen_total += loss_gen
    loss_dis_total += loss_dis
  ...
  return total_err, joint_err, step_time, loss_gen_total, loss_dis_total
...
def sample():
  ...
  with tf.Session(config=tf.ConfigProto( device_count = device_count )) as sess:
    # === Create the model ===
    print("Creating %d gen and %d dis layers of %d units." % (FLAGS.num_layers_gen, FLAGS.num_layers_dis, FLAGS.linear_size))
    batch_size = 128
